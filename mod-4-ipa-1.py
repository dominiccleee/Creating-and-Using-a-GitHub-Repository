...
def tic_tac_toe(board):
    ...
    for a, b in enumerate(rows):
        if all([i == "X" for i in b]):
            return "X"
        elif all([i == "O" for i in b]):
            return "O"
        else:
            continue

    for c, d in enumerate(columns):
        if all([i == "X" for i in d]):
            return "X"
        elif all([i == "O" for i in d]):
            return "O"
        else:
            continue

    if all([i == "X" for i in updown_diagonal]):
          return "X"
    elif all([i == "O" for i in updown_diagonal]):
          return "O"
    elif all([i == "X" for i in downup_diagonal]):
          return "X"
    elif all([i == "O" for i in downup_diagonal]):
          return "O"
    else:
          return "NO WINNER"
...
def eta(first_stop, second_stop, route_map):
    ...
    for i in c:
        p = c.index(i)
        if c[p] == first_stop and d[p] == second_stop: 
            one_stop = int(route_map[c[p], d[p]]['travel_time_mins'])
            return one_stop